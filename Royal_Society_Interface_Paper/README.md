...
| `all_figures/` | The twelve figures, `f1`-`f8` main text and `fa1`-`fa4` appendix, **generated** |

Nothing in `all_figures/` and none of the three `*_auto.tex` files is written by hand.
...
The pipeline lives in `../Analysis/scaling/`. It writes into whichever manuscript
directory two environment variables name, defaulting to `../paper_scaling/` so the
other manuscript keeps building unchanged. To target **this** directory:

```bash
cd ../Analysis/scaling
export PD_PAPER_DIR="$(pwd)/../../Royal_Society_Interface_Paper"
export PD_FIGDIR="$PD_PAPER_DIR/all_figures"
