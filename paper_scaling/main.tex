...
% Three numerals only: 200{,}000, 0.636, and the "at most 1" threshold.
%
...
by theorem rather than convention. Whether language models respect that
...
200{,}000 decisions. Cooperation moves in every model, by up to 0.636
(95\% CI 0.588, 0.686), and the movement has a different shape in each, so
averaging over models reports a movement no model in the average makes. Its
size depends on the prompt language. The scale also gates the persona
instruction, reversing its sign in two models of five where every printed payoff
is at most 1. In four models of five what is played changes too, becoming less
consistent with any canonical memory-one rule; a sixth model, in the
supplement, moves the other way. The effect is already present on
the opening move. Cooperation rates reported for language models should
therefore be interpreted together with their sensitivity to payoff scale.
\end{abstract}
...
% and "0.636" (contribution 1).  Both already appear elsewhere in the
...
a fast-growing literature measures how they behave when they do: through
behavioural economics and cognitive
psychology~\citep{binz2023using,horton2023large,aher2023using}, and through the
classical games~\citep{mei2024turing}. The prisoner's dilemma is the workhorse,
its strategy space mapped and its predictions
...
We ask four questions of the rescaling. Does strategic behaviour survive
multiplying every payoff by a positive constant, and by how much does it fail
to? Does the answer depend on the prompt language, and on the persona the model
is told to adopt? Does the rescaling change only how often a model cooperates,
or the strategy it is playing? And does the effect enter at the opening move or
through the course of play? We answer them on a complete and exactly balanced
grid of 10{,}000 dyads (\S\ref{sec:methods}).
...
to 0.636 in cooperation. Second, we show that the departure has a different
shape in each model, to the point of anticorrelation, so that what a pooled
analysis reports is a movement no model in the pool makes: the interaction of
the payoff scale with the model carries $17.9\%$ of the variance across cells
against $5.8\%$ for the main effect, and averaging the five curves gives a
range of 0.154 where the mean within-model range is 0.249. The main effect is
significant here only because one model departs far enough to survive the
averaging, which is a fact about that model rather than a property of language
models in general, and a panel assembled without it returns no significant main
effect at all. The same holds one level down, since the size of the departure
depends on the prompt language as well as on the model.
...
canonical memory-one rule in four models of five while the fifth shows no
trend, and the effect is already present on the opening
...
agent playing the game is exactly zero. An agent that instead applies a
non-linear valuation to years of imprisonment is not bound by the invariance;
...
years of imprisonment to be minimized.
