...
CI_OUT = PAPER_DIR / "conditioning_ci_auto.tex"
...
def s10_conditioning_ci(t20):
    """Generate the robustness table from the whole-dyad CI ledger."""
    t = t20.set_index(["model", "contrast"])
    rows = []
    for m in MODEL_ORDER_ALL:
        cells = []
        for k in ("reciprocity", "persistence", "matching"):
            x = t.loc[(m, k)]
            cells.append(f"${x.estimate:.3f}\\,[{x.lo:.3f},\\,{x.hi:.3f}]$")
        rows.append(f"{MODEL_LABEL[m]} & " + " & ".join(cells) + " \\\\")
    return wrap("\n".join(rows),
                "\\textbf{Whole-dyad bootstrap intervals for memory-one contrasts.} "
                "Entries are estimates with 95\\% percentile intervals. Each "
                "model contributes 2,000 dyads and each interval uses 4,000 "
                "bootstrap draws.",
                "tab:conditioning-ci", "lccc",
...
def main():
    ...
    CI_OUT.write_text(
        ...
        + s10_conditioning_ci(t20), encoding="utf-8")
    print(f"wrote {OUT}")
